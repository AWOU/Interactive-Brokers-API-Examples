...
def super_order(parentOrderId: int, action: str, quantity: float, current_price: float):

    # The time to cancel the order if it doesn't fill
    cancel_time_dt = dt.datetime.now() + dt.timedelta(minutes=3)
    cancel_time = cancel_time_dt.strftime("%Y%m%d %H:%M:%S")
    ...
    parent.orderType = "LMT"
    parent.totalQuantity = quantity
    parent.lmtPrice = round(current_price * 1.001, 2) if action == "BUY" else round(current_price * 0.999, 2)
    # The new stuff I added to make it cancel if after a certain time if the order doesn't fill
    parent.tif = "GTD"
    parent.goodTillDate = cancel_time
    parent.transmit = False

    # The cancel time for the order to allow for the next one
    takeProfitCancelTime = dt.datetime.now() + dt.timedelta(minutes=16)
    tpCancelTime = takeProfitCancelTime.strftime("%Y%m%d %H:%M:%S")
    # The original profit taking order
    takeProfit = Order()
    takeProfit.orderId = parent.orderId + 1
    takeProfit.action = "SELL" if action == "BUY" else "BUY"
    takeProfit.orderType = "LMT"
    takeProfit.totalQuantity = quantity
    takeProfit.lmtPrice = current_price * 1.004
    takeProfit.parentId = parentOrderId
    parent.tif = "GTD"
    parent.goodTillDate = tpCancelTime
    takeProfit.transmit = False
    ...
    stopLoss.orderType = "STP"
    # Stop trigger price
    stopLoss.auxPrice = round(current_price * .996, 2) if action == "BUY" else round(current_price * 1.004, 2)
    stopLoss.totalQuantity = quantity
    stopLoss.parentId = parentOrderId
    # In this case, the low side order will be the last child being sent. Therefore, it needs to set this attribute to True
    # to activate all its predecessors
    ...
    # These are the new orders that limit the price of the stock each time interval
    takeProfit2Time = dt.datetime.now() + dt.timedelta(minutes=15)
    profit2Time = takeProfit2Time.strftime("%Y%m%d %H:%M:%S")
    # The cancel time for the order to allow for the next one
    takeProfit2CancelTime = dt.datetime.now() + dt.timedelta(minutes=31)
    tp2CancelTime = takeProfit2CancelTime.strftime("%Y%m%d %H:%M:%S")

    takeProfit2 = Order()
    takeProfit2.orderId = parent.orderId + 3
    takeProfit2.goodAfterTime = profit2Time
    takeProfit2.action = "SELL" if action == "BUY" else "BUY"
    takeProfit2.orderType = "LMT"
    takeProfit2.totalQuantity = quantity
    takeProfit2.lmtPrice = round(current_price * 1.003, 2) if action == "BUY" else round(current_price * 0.997, 2)
    takeProfit2.parentId = parentOrderId
    parent.tif = "GTD"
    parent.goodTillDate = tp2CancelTime
    takeProfit2.transmit = False

    takeProfit3Time = dt.datetime.now() + dt.timedelta(minutes=30)
    profit3Time = takeProfit3Time.strftime("%Y%m%d %H:%M:%S")
    # The cancel time for the order to allow for the next one
    takeProfit3CancelTime = dt.datetime.now() + dt.timedelta(minutes=46)
    tp3CancelTime = takeProfit3CancelTime.strftime("%Y%m%d %H:%M:%S")

    takeProfit3 = Order()
    takeProfit3.orderId = parent.orderId + 4
    takeProfit3.goodAfterTime = profit3Time
    takeProfit3.action = "SELL" if action == "BUY" else "BUY"
    takeProfit3.orderType = "LMT"
    takeProfit3.totalQuantity = quantity
    takeProfit3.lmtPrice = round(current_price * 1.002, 2) if action == "BUY" else round(current_price * 0.998, 2)
    takeProfit3.parentId = parentOrderId
    parent.tif = "GTD"
    parent.goodTillDate = tp3CancelTime
    takeProfit3.transmit = False

    # If none of the other orders get filled by the exit_time then the order will execute on its own
    exit_time_dt = dt.datetime.now() + dt.timedelta(hours=1)
    exit_time = exit_time_dt.strftime("%Y%m%d %H:%M:%S")
    timeOrder = Order()
    timeOrder.orderId = parent.orderId + 6
    timeOrder.goodAfterTime = exit_time
    timeOrder.action = "SELL" if action == "BUY" else "BUY"
    timeOrder.totalQuantity = quantity
    timeOrder.orderType = "MKT"
    ...
    bracketOrder = [parent, takeProfit, stopLoss, takeProfit2, takeProfit3, timeOrder]
    return bracketOrder
